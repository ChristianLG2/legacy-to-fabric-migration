...
Bronze (raw ingestion, audit lineage) → Silver (cleaned, typed, quarantine framework for invalid records) → Gold (Kimball star schema, 3 dimensions, 3 fact tables) → Direct Lake semantic model → Power BI report. Full technical detail in the main [README](../README.md).